...
def test_post_login_login_without_csrf(testapp_admin):
    with patch('play.administration.account.render_template', Mock(return_value='template')) as template:
        response = testapp_admin.post('/account/login', {'username': '12345', 'password': '12345'})
    assert response.status_code == 200
    assert template.call_args_list[0][1]['form'].errors == {'csrf_token': ['CSRF token missing']}


def test_post_wrong_login_with_csrf(testapp_admin):
    with patch('play.administration.account.render_template', Mock(return_value='template')) as template:
        response = testapp_admin.post('/account/login', OrderedDict([('csrf_token', 'sometoken'), ('password', 'password'), ('username', 'admin_active')]))
    assert response.status_code == 200
    assert template.call_args_list[0][1]['form'].errors == {'csrf_token': ['CSRF token missing']}
...
def test_post_correct_login_with_csrf(testapp_admin):
    with patch('flask.ext.wtf.form.validate_csrf', Mock(return_value=True)):
        response = testapp_admin.post('/account/login', OrderedDict([('password', 'password'), ('username', 'admin_active')]))
    assert response.status_code == 302


@mark.parametrize('username_password', [('admin_active', 'password-wrong'), ('user_active', 'password'),  ('user_not_existent', 'password')])
def test_post_wrong_login(testapp_admin, username_password):
    username, password = username_password
    with patch('play.administration.account.render_template', Mock(return_value='template')) as template:
        with patch('flask.ext.wtf.form.validate_csrf', Mock(return_value=True)):
            response = testapp_admin.post('/account/login', OrderedDict([('password', password), ('username', username)]))
    assert response.status_code == 200
    assert 'Username/password combination unknown' in template.call_args_list[0][1]['form'].errors['username']


def test_post_redirect_for_logged_in_user(testapp_admin):
    with patch('play.administration.account.current_user', Mock(is_authenticated=True)) as current_user:
        response = testapp_admin.post('/account/login', OrderedDict([('password', 'password'), ('username', 'admin_active')]))
    assert response.status_code == 302
...
def test_logout_logged_in(testapp_admin):
    with patch('flask.ext.wtf.form.validate_csrf', Mock(return_value=True)):
        response_login = testapp_admin.post('/account/login', OrderedDict([('password', 'password'), ('username', 'admin_active')]))
    response_home = testapp_admin.post('/account/login', OrderedDict([('password', 'password'), ('username', 'admin_active')]))
